...
app.post("/insert",(req,res)=>{
   ...
   console.log(pista)
   insert(connection,{maxJugadores:4,cantJugadores:0,pinAcceso:pin,turnoJugador:0,ganador:0,Frase:frase,Pista:pista},(result)=>{
        //res.json(result);
   ...
   read(
      connection,{PIN:pin,jugador:alias},(result)=>{
         ...
         if(result=="partida llena"||result=="partida inexistente"){
            ...
            aliasJugador= alias;
            nombreJugadores.push(alias);
            for( var i =0; i < nombreJugadores.length ; i++){
               console.log("nombre jugador "+ i+": "+nombreJugadores[i]);
...
io.on('connect',function(socket){
   ...
   socket.on("menuJugadores", function(){
      for( var i =1; i < listaJugadores.length ; i++){
         io.to(listaJugadores[i]).emit("menuJugadores");
