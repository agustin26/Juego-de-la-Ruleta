...
function actualizarDatosPantalla() {
 ...
 // nodoIntentosOriginales.textContent = numIntentosOriginales;
  ...
  nodoPuntuacionJ1.textContent =  " Jugador 1: "+ puntuacionJ1 + " puntos";
  };
  if(jugadores.length==3){
     // Mostramos la puntuación del usuario
  nodoPuntuacionJ1.textContent = " Jugador 1: "+ puntuacionJ1 + " puntos";
  nodoPuntuacionJ2.textContent =" Jugador 2: " + puntuacionJ2+ " puntos";
  }
  ...
  nodoPuntuacionJ1.textContent = " Jugador 1: "+ puntuacionJ1 + " puntos";
  nodoPuntuacionJ2.textContent =" Jugador 2: " + puntuacionJ2+ " puntos";
  nodoPuntuacionJ3.textContent =" Jugador 3: " +puntuacionJ3+ " puntos";
    }
...
function resolverFrase(){
  ...
  if (fraseAdivinada != palabraAdivinarString) {
    fraseVentana="La frase que ingreso Jugador"+[turno]+" es incorrecta.";
    ...
    fraseVentana= "Es el turno de  Jugador "+[turno];
    socket.emit("fraseVentana", fraseVentana);
  }
  estadoPartida();
...
function estadoPartida() {
  ...
  if (!palabraMostrar.includes('_')) {
    fraseVentana="¡Enhorabuena, Jugador"+[turno]+" has ganado!" ;
    palabraMostrar = palabraAdivinar;
    socket.emit("fraseVentana", fraseVentana);
    // Bloqueamos todas las teclas para que el usuario no pueda clickar las restantes
    bloquearTodasTeclas();
...
function reiniciarPartida() {
 //ennviamos quien comenzo partida
  partidaIniciada=1;
  socket.emit('comenzoPartida',{turno:turno, partidaIniciada:partidaIniciada});
...
function stopRotateWheel() {
  ...
  if(valorRuleta!=0 && valorRuleta!="bancarrota"){
    ...
    fraseVentana="Jugador "+[turno]+" Debe elegir una consonante por "+ valorRuleta +" puntos.";
    ...
    puntuacion=0;
    fraseVentana="Mala suerte, la puntuacion de Jugador"+[turno]+ "volvera a 0.";
    socket.emit("fraseVentana", fraseVentana);
...
socket.on("usuarioDesconectado", function(dato){
  fraseVentana= "Jugador "+[turno]+" se ah desconectado.";
  abrirVentanainfo();
});
...
function mostrarFraseVentana(){
  ...
  socket.on('listaJugadores', function(dato){
      ...
      if(jugadores.length  ==1){
       ...
       if(jugadores.length>=2  ){
        console.log("la partida se ha iniciado y su valor es = "+ partidaIniciada);
        if(partidaIniciada==0){
        nodoJugadoresCon.textContent = "Jugadores conectados: "+(jugadores.length -1 );
        socket.emit("menuJugadores",{jugador: jugadores.length -1 });
       
       }if(partidaIniciada==1 ){
        
        socket.emit("actualizarPantallaJugNuevo");
       }
      }
      //cosas del admin
       idUsuario= socket.id;
...
socket.on('turno', function(dato){
      if(jugadores.length >2){
        fraseVentana="Es el turno de Jugador "+dato;
        abrirVentanainfo();
      } 
...
socket.on("actualizarPantallaJugNuevo", function(datos){
  palabraMostrar= datos.palabraMostrar;
  palabraAdivinar= datos.palabraAdivinar;
  turno= datos.turno;
  puntuacionJ1= datos.puntuacionJ1;
